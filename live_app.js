(function () {
  ...
  function resolveTextTokens(text) {
    ...
    const replacements = {
      '[????FO??]': config.contact && config.contact.phone ? config.contact.phone : '[????FO??]',
      ...
      '[?????????S]': config.gift && config.gift.beneficiary ? config.gift.beneficiary : '[?????????S]',
      '[RSVP_DEADLINE]': config.rsvp && config.rsvp.deadline ? config.rsvp.deadline : '[RSVP_DEADLINE]',
      '[?????S??_?_?O??S]': config.event && config.event.venue ? config.event.venue : '[?????S??_?_?O??S]',
      '[????T??S?]': config.event && config.event.address ? config.event.address : '[????T??S?]',
    };
  ...
  function getPlaceQuery(place) {
    return encodeURIComponent(place.address || place.name || '??p??es?a e?d???s??');
  }
  ...
  const els = {
    ...
    introMonogram: $('#introMonogram'),
    introNameOne: $('#introNameOne'),
    introNameTwo: $('#introNameTwo'),
    introDate: $('#introDate'),
  ...
  function injectText() {
    ...
    if (els.introMonogram) els.introMonogram.textContent = displayMonogram;
    ...
    els.brandMonogram.textContent = displayMonogram;
    ...
    els.rsvpDeadlineText.textContent = `?a?a?a???µe e??µe??ste µa? ??? ${resolveTextTokens(config.rsvp.deadline)}.`;
        ...
        return `
          <div class="map-visual glass-card">
            <div class="map-caption">
              <p class="panel-label">${resolveTextTokens(place.label || '??p??es?a')}</p>
              <h3>${resolveTextTokens(place.name || '??p??es?a')}</h3>
            ...
            <div class="map-embed">
              ${
                hasEmbed
                  ? `<iframe
                      title="${resolveTextTokens(place.name || place.label || '???t?? t?p??es?a?')}"
                      loading="lazy"
                  ...
                  : `<div class="map-fallback">
                      <div>
                        <p class="eyebrow">??p??es?a</p>
                        <h3>? p??ep?s??p?s? ???t? ?a eµfa??ste? ed?</h3>
                        <p>????? p??ste?e? t? embed t?? ???t?, ?a eµfa??ste? se a?t? t? s?µe??.</p>
                      </div>
                    </div>`
              }
            </div>
            <div class="map-actions">
              <button class="btn btn-primary" type="button" data-open-map="${place.id}">?????µa st?? ???t?</button>
              <button class="btn btn-secondary" type="button" data-copy-location="${place.id}">??t???af? st???e???</button>
            </div>
          </div>
        `;
      })
  ...
  function openMapChoiceModal(placeId) {
    ...
    els.mapChoiceDescription.textContent = `?p????te p?? ???ete ?a a????ete t? d?ad??µ? ??a ${resolveTextTokens(place.name || place.label || 't?? t?p??es?a')}.`;
  ...
  function setupLocations() {
    ...
    els.locationMaps.addEventListener('click', (event) => {
      ...
      copyText(getPlaceCopyText(place), '?a st???e?a t?p??es?a? a?t????f??a?.');
    });
  ...
  function getGiftStatusMeta(status) {
    if (status === 'reserved') {
      return {
        ...
        actionLabel: '?? d?a??s?µ?',
        description: '?? d??? ??e? ?d? desµe?te? ?a? de? µp??e? ?a ep??e?e? ?a??.',
      };
    }
    ...
    if (status === 'purchased') {
      return {
        ...
        actionLabel: '???????????e',
        description: '?? d??? ??e? ?d? a???aste? ?a? de? e??a? p???? d?a??s?µ?.',
      };
    }
    ...
    return {
      ...
      actionLabel: '??sµe?s? d????',
      description: '???te p??ta d?sµe?s? ?a? µet? a????te t? ?at?st?µa ??a ?a ap?f????µe d?p?? a????.',
    };
  }
  ...
  async function reserveGiftItem(payload) {
    ...
    if (registry.fallbackMode === 'local') {
      ...
      if (existing && existing.status && existing.status !== 'available') {
        const error = new Error('?? d??? ??e? ?d? desµe?te?.');
    ...
    throw new Error('?e? ??e? s??de?e? backend ??a t? ??sta d????.');
  }
  ...
  function renderGiftRegistrySummary() {
    ...
    const modeLabel = registry.endpoint ? 'S???????sµ??? ??sta' : '??sta d?a??s?µ?';

    els.registryCard.style.display = '';
    els.registryCard.innerHTML = `
      <p class="panel-label">??sta ?atast?µ?t??</p>
      ...
      <button class="btn btn-secondary" type="button" data-open-gift-list>??sta d????</button>
    `;
  }
    ...
    els.giftRegistry.innerHTML = items
      .map((item) => {
        ...
        const priceMeta = item.lastCheckedAt
          ? `<p class="gift-registry-meta">?e?e?ta?a e??µ???s? t?µ??: ${item.lastCheckedAt}</p>`
          : '';
        ...
        return `
          <article class="gift-registry-item ${statusMeta.className}">
            <div class="gift-registry-head">
              <div>
                <p class="gift-registry-category">${item.category || '???? ??µ??'}</p>
                <h3>${item.title}</h3>
              </div>
  ...
  function validateGiftReservationForm(payload) {
    ...
    if (!payload.guestName.trim()) {
      errors.name = '?a?a?a???µe s?µp????ste t? ???µ? sa?.';
    }

    if (!payload.guestContact.trim()) {
      errors.contact = 'S?µp????ste t???f??? ? email ??a ?a ?p???e? st???e?? ep????????a?.';
    }
  ...
  function openGiftReserveModal(itemId) {
    ...
    els.giftReserveDescription.textContent = `Ta desµe?sete t? "${item.title}" ap? ${item.store}. ?et? t? d?sµe?s? µp??e?te ?a a????ete t? ?at?st?µa ap? t? ???µp? p?? ?a eµfa??ste?.`;
  ...
  async function handleGiftReserveSubmit(event) {
    ...
    if (Object.keys(errors).length > 0) {
      setGiftReservationStatus('?a?a?a???µe d?????ste ta ped?a t?? f??µa?.', 'error');
      return;
    }

    els.giftReserveSubmit.disabled = true;
    els.giftReserveSubmit.textContent = '??sµe?s?...';
    ...
    try {
      ...
      renderGiftRegistrySummary();
      renderGiftRegistry();
      setGiftReservationStatus(getGiftRegistryConfig().successMessage || '?? d??? desµe?t??e ep?t????.', 'success');
      els.giftStoreLink.hidden = false;
      els.giftStoreLink.href = item.url;
      showToast('?? d??? desµe?t??e.');
    } catch (error) {
      if (error.code === 'ALREADY_RESERVED' || error.code === 'CONFLICT') {
        await loadGiftReservations().catch(() => {});
        renderGiftRegistrySummary();
        renderGiftRegistry();
        setGiftReservationStatus('?? d??? desµe?t??e µ???? ap? ????? ep?s??pt? ?a? de? e??a? p?a d?a??s?µ?.', 'error');
      } else {
        setGiftReservationStatus('?e? ???????????e ? d?sµe?s?. ????µ?ste ?a?? se ????.', 'error');
      }
    } finally {
      els.giftReserveSubmit.disabled = false;
      els.giftReserveSubmit.textContent = '??sµe?s? d????';
    }
  }
  ...
  async function copyText(text, successMessage) {
    try {
      ...
      showToast('?e? ?ta? d??at? ? a?t???af?.');
    }
  }

  function setupCopyButtons() {
    els.copyIbanButton.addEventListener('click', () => copyText(config.gift.iban, '?? IBAN a?t????f??e.'));
    els.copyBeneficiaryButton.addEventListener('click', () => copyText(`${config.gift.beneficiary}\n${config.gift.iban}`, '?a st???e?a a?t????f??a?.'));
  }
  ...
  function setupCalendarButton() {
    ...
    els.addToCalendarButton.addEventListener('click', () => {
      try {
        ...
        showToast('?? a??e?? ?µe???????? e??a? ?t??µ?.');
      } catch (error) {
        showToast('?e? ?ta? d??at? ? d?µ??????a t?? a??e??? ?µe????????.');
      }
    });
  }
  ...
  function validateForm(formData) {
    ...
    if (!Number.isInteger(guests) || guests < 1 || guests > 12) {
      errors.guests = '?p????te a???µ? at?µ?? ap? 1 ??? 12.';
    }

    if (!formData.attendance) {
      errors.attendance = '?a?a?a???µe ep????te a? ?a pa?e??e?e?te.';
    }

    if (!formData.privacy) {
      errors.privacy = '??e???eta? ? s???at??es? sa? ??a t?? ap?st??? t?? f??µa?.';
    }
  ...
  function setupRsvpForm() {
    els.rsvpForm.addEventListener('submit', async (event) => {
      ...
      if (Object.keys(errors).length > 0) {
        setFormStatus('?a?a?a???µe d?????ste ta ep?s?µasµ??a ped?a.', 'error');
        return;
      }

      const submitButton = els.rsvpForm.querySelector('button[type="submit"]');
      submitButton.disabled = true;
      submitButton.textContent = '?p?st???...';
      ...
      try {
        ...
        setFormStatus('?a???s??st??e p??ß??µa ?at? t?? ap?st???. ????µ?ste ?a?? ? ep????????ste µa?? µa?.', 'error');
      } finally {
        submitButton.disabled = false;
        submitButton.textContent = '?p?st??? ep?ßeßa??s??';
      }
    });
  }
